        ...
        <template v-if="item.type === 1">
          <el-submenu :index="item.id + ''">
            ...
            <template v-for="children in item.children" :key="children.id">
              <el-menu-item
                :index="children.id + ''"
                @click="handleClick(children.url)"
              >
...
import { mapRoutes, pathMapToMenu } from '@/utils/asyncRouter'
...
export default defineComponent({
  ...
  setup() {
    ...
    const menu = pathMapToMenu(useMenus.value, current)
    // console.log(pathMapToMenu(useMenus.value, current))
